...
class sqlite_testclass : public CxxTest::TestSuite
{
  ...
  public:
    ...
    {
        TS_ASSERT_THROWS_EQUALS(sqlite_db("/tmp/test.db", false), const sqlite_exception & e, e.what(),
                                "Can not open database '/tmp/test.db': unable to open database file");
    }

    void test_simpleio()
    {
        sqlite_statement create_table_stmt{ "CREATE TABLE `test_table`"
                                            "("
                                            "PersonID int,"
        ...
        sqlite_statement{ "INSERT INTO `test_table`"
                          "VALUES"
                          "("
        ...
        sqlite_statement get1{ "SELECT * FROM `test_table`"
                               "WHERE PersonID = 1701;",
                               *m_database };
